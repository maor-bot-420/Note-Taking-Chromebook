...
if (presetPalette) {
  presetColors.forEach(color => {
    ...
    if (colorPicker && color === colorPicker.value) swatch.classList.add('selected');
...
const cmInPixels = Math.round(cmHelper.getBoundingClientRect().width) || 38;
...
/* ==========================================
   SHAPE RECOGNITION & SCRIBBLE GESTURES
   ========================================== */
...
/* ==========================================
   PAGE CLASS DEFINITION & RENDERING
   ========================================== */
class Page {
  constructor(index) {
    this.index = index;
    ...
    if (zoomWrapper) zoomWrapper.appendChild(this.container);
  ...
  render() {
    ...
    if (isDrawing && activePage === this && isShapeSnapped && snappedShape) {
      ...
      this.ctx.strokeStyle = colorPicker ? colorPicker.value : '#222222';
  ...
  attachEvents() {
    ...
    // Pointer events for physical input hardware
    ...
    // Explicit touch event listeners to override browser touch scrolling takeover
...
function applyZoom(newZoom) {
  ...
  if (zoomValText) zoomValText.textContent = `${Math.round(zoomLevel * 100)}%`;
}
...
function setupToolStyle() {
  pages.forEach(page => {
    ...
    const selectedColor = colorPicker ? colorPicker.value : '#222222';
...
function setTool(toolName, selectedBtn) {
  ...
  toolBtns.forEach(btn => { if (btn) btn.classList.remove('active'); });
...
function startDrawing(e, page) {
  ...
  // In Pen mode: stylus draws, finger touches scroll
...
function restoreFreehandStroke(page) {
  ...
  page.mainCtx.save();
  page.mainCtx.strokeStyle = colorPicker ? colorPicker.value : '#222222';
...
function draw(e, page) {
  ...
  if (isScribble) {
    ...
    if (currentTool === 'marker') {
      page.strokeCtx.strokeStyle = colorPicker ? colorPicker.value : '#222222';
...
function stopDrawing(page, e) {
  ...
  if (currentTool === 'strokeEraser') {
    ...
    const newStroke = {
      tool: 'shape',
      shape: snappedShape,
      color: colorPicker ? colorPicker.value : '#222222',
...
/* ==========================================
   SESSION, FILE SAVE & LOAD CONTROLS
   ========================================== */
function startSession(title, size, bg) {
  ...
  if (bgSelectToolbar) bgSelectToolbar.value = bg;
  if (activeNoteTitle) activeNoteTitle.textContent = noteConfig.title;
...
function updateBackgroundStyle(newBg) {
  noteConfig.bg = newBg;
  if (bgSelectToolbar) bgSelectToolbar.value = newBg;
...
function openFile(event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (bgSelectToolbar) bgSelectToolbar.value = noteConfig.bg;
      if (activeNoteTitle) activeNoteTitle.textContent = noteConfig.title;
      if (zoomWrapper) zoomWrapper.innerHTML = '';
      pages = [];
...
if (zoomInBtn) zoomInBtn.addEventListener('click', () => applyZoom(zoomLevel + 0.15));
if (zoomOutBtn) zoomOutBtn.addEventListener('click', () => applyZoom(zoomLevel - 0.15));
if (resetZoomBtn) resetZoomBtn.addEventListener('click', () => applyZoom(1.0));

if (createNoteBtn) {
  createNoteBtn.addEventListener('click', () => {
    const title = noteTitleInput ? noteTitleInput.value.trim() : 'My Note';
    const size = initPageSizeSelect ? initPageSizeSelect.value : 'a4';
    const bg = initBgSelect ? initBgSelect.value : 'grid-1cm';
...
if (openNoteBtn) openNoteBtn.addEventListener('click', () => openFileInput && openFileInput.click());
if (openFileInput) openFileInput.addEventListener('change', openFile);
...
if (markerBtn) markerBtn.addEventListener('click', () => setTool('marker', markerBtn));
if (strokeEraserBtn) strokeEraserBtn.addEventListener('click', () => setTool('strokeEraser', strokeEraserBtn));
